...
- `Dockerfile`: the only build artifact. An Alpine `builder` stage compiles
  darkhttpd statically (hardening flags + UPX), a `test` stage runs the smoke
  test, and the final stage ships the single binary on `scratch`.
...
`DL3018` (unpinned `apk add` for `build-base`/`upx`/`binutils`) is
intentionally ignored inline in the `Dockerfile`; those packages are build-only
and never ship (see Conventions), so don't add a version pin or a hadolint
...
- The image runs non-root by default (`USER 65534:65534`). Keep it that way;
  darkhttpd needs no root (high port, read-only), and compose `user:` can
  override it at runtime.
- Match the existing dependency-pinning scheme: the base image is pinned by
  digest and darkhttpd by version + SHA-256. The build-only
  `build-base`/`upx`/`binutils` packages are intentionally left unpinned (they
  never reach the scratch final image); `DL3018` is ignored inline for exactly
  this reason, so don't add a version pin to "fix" it.
